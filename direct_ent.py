...
from tools import comp_frame, hamming, kraus_dep, overlap, pauli6_frame, \
    povm_meas, random_lpdo, random_rotation_site, rotate_lpdo
import argparse
import os
...
if __name__=="__main__":
        ...
        if not os.path.exists('./direct_data_ent'):
            os.mkdir('./direct_data_ent')
        data = []
        data.append(error_inn)
        data.append(error_quasi)
        data.append(over)
        data = np.array(data)
        np.save(f'./direct_data_ent/ent_qubits_{"_".join([str(q) for q in m_range])}_{instance_number}inst_nu{N_U}_nm{N_M}', data)
