...
# -*- coding: utf-8 -*-
"""
Calculates the minimum number of samples required to estimate the overlap
between pairs of random pure entangled states within an error bound.

"""

import numpy as np
import time
from tools import comp_frame, overlap, pauli6_frame, povm_meas, \
    random_lpdo, rotate_lpdo 
import argparse
import os
...
if __name__=="__main__":
        ...
        for m in m_range:
            ...
            while av_err > error_bound:
                ...
                av_err = np.mean(err)
                st_dev = np.std(err)
                print('samples', nsamples, 'st dev', st_dev, 'av', av_err)
            quasi.append([nsamples, av_err, st_dev])
            sample_final_quasi.append(nsamples)

        if not os.path.exists('./scaling_ent'):
            os.mkdir('./scaling_ent')
        sample_final = np.array([quasi, sample_final_quasi])
        np.save(f'./scaling_ent/ent_scaling_{"_".join([str(q) for q in m_range])}_{instance_number}inst_{nrep}rep', sample_final)
