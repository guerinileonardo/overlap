...
import numpy as np
import time
from tools import pauli6_frame_tn, random_mps, overlap4, eff_meas, \
    BB_block, trace, dep_site, kraus_dep, rotate_mps, renorm 
...
for instance in range(instance_num):
    ...
    psi.append(random_mps(m, pd))
    phi.append(rotate_mps(psi[instance], 
                          (np.mod(instance, 3)+1)*np.pi/8,
                          cnot_on=False))
    ov = overlap4(psi[instance], phi[instance])
    E.append(ov)
