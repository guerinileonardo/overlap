...
implementation of the improved swap test circuit (via the LPDO paradigm) and
...
import numpy as np
import time
from tools import pauli6_frame, random_lpdo, overlap, eff_meas, \
    dep_site, kraus_dep, rotate_lpdo, renorm, ist_block, meas
...
if __name__=="__main__":
        ...
        for instance in range(instance_num):
            ...
            psi.append(random_lpdo(m, pd, 2))
            phi.append(rotate_lpdo(psi[instance], 
                                  angle,
                                  cnot_on=False))
            ov = overlap(psi[-1], phi[-1])
            E.append(ov)
